...
- **Local-only.** Deployment has been decommissioned to stop Container/R2/AI-Search spend: there is no deployed worker, and the `deploy`/`predeploy`/`tail` scripts are removed. Everything runs against local Miniflare via `bun run dev` / `bun run dev:local` (Cloudflare's own docs describe no scenario where `wrangler dev` reaches remote infra, so local iteration is free). Re-enabling deploy means restoring those scripts and re-provisioning the account resources (R2 buckets, AI Search instance, secrets) — see README "Deploy".
- `bun run dev` — `wrangler dev` (needs Docker). `bun run cf-typegen` — `wrangler types`.
...
- Secrets: `OPENROUTER_API_KEY` (model calls) + `API_TOKEN` (the Authorization bearer; the FE's `VITE_API_TOKEN` is inlined at build time and must match) + `SECRETS_ENCRYPTION_KEY` (AES-GCM key for session-stored secrets, `apps/api/src/SecretsCrypto.ts`) come from `.dev.vars` locally (template: `.dev.vars.example`). `.dev.vars` must exist BEFORE `bun run typecheck` — the generated `Env` derives its keys from it. (`wrangler secret put …` applies only if you re-enable deploy.)
- Re-enabling deploy (removed for local-only development) needs, once per account: `wrangler r2 bucket create efflux-skills` + `efflux-sessions`, `wrangler ai-search create efflux-knowledge --type builtin`, the secrets above via `wrangler secret put`, restoring the `deploy`/`predeploy` scripts, and re-adding `"remote": true` to the `ai_search` binding. A `remote: true` binding blocks `wrangler dev` until its instance exists — see `ISSUES.md`.
- Scheduled-job outcome notifications (issue #137): jobs can alert on completion/failure via the `notify` param on `create_scheduled_job`. Slack posts to a webhook URL stored as a session secret (no infra). Email uses the `send_email` binding (`EMAIL`) in `wrangler.jsonc` and the `NOTIFY_EMAIL_FROM` var — set that var (per-env) to a verified sender on the account's Email Routing domain, and the recipient (`emailTo`) must be a verified Email Routing destination; email is left unset (`""`) by default and rejected at job-creation until configured. The `send_email` binding is intentionally non-`remote`, so `wrangler dev` does not send real email — verify email on the deployed worker.
...
- Verification = exercised against a running **local** worker (`/efflux-verifying` checklist) — typecheck/build alone NEVER counts. `bun run dev:local` (or `bun run dev`) runs Containers entirely through the local Docker daemon — Cloudflare's own docs describe no scenario where it reaches remote infrastructure — so local iteration is free. There is no deployed worker anymore: the billed Container/R2/AI-Search usage was the whole reason it was decommissioned (see ISSUES.md). If you ever re-enable deploy, treat every deploy as real, shared, billed usage and never debug by redeploying.
- Branch flow: feature PRs target `staging` with `Refs #N`; `/efflux-releasing` live-validates the promoted batch against a running worker, then promotes staging→main with a PR carrying the `Closes #N` lines (issues close on release, not on feature landing) and recording the per-feature validation in its body. A release is cut only after that live pass — CI-green/typecheck alone never certifies one. Merge commits only — never rebase/squash. Never `--no-verify`.
...
- `/efflux-releasing` — live-validate the promoted batch against a running worker (per `/efflux-verifying`), then promote `staging`→`main` via a merge PR carrying the `Closes #N` lines (and a per-feature validation table), then tag + GitHub-release the merge; semver from 0.1.0, pre-1.0 rules (feat → minor, everything else → patch).
