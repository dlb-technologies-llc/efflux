...
/** Default for the cross-session memory system-prompt injection when a session stores no `memoryEnabled` — on by default; the memory tools themselves are governed by the rules table. */
...
export const resolveConfig = (stored: typeof AgentConfig.Type): ResolvedConfig => ({
  ...
  memoryEnabled: stored.memoryEnabled ?? DEFAULT_MEMORY_ENABLED,
})
