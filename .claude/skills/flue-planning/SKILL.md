...
- **Execution model:** `/flue-executing` runs the plan in an isolated worktree (`.claude/worktrees/<plan-name>`, branched from `origin/main`) and spawns each wave's tasks as PARALLEL `flue-task-executor` agents. Consequences for the plan: the `> **Branch:**` metadata line is REQUIRED (optionally `> **Base:**` when the work builds on an open PR branch instead of `main`); no two tasks in the same wave may touch the same file (they run concurrently); and each task body must be SELF-CONTAINED — sub-agents reliably read their prompt but unreliably read referenced files, so inline any binding pattern or `**Do not:**` list into the task body rather than pointing at another document.
