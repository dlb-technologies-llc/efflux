...
4. **Draft.** Use the plan template: a `# Plan:` header with `> **Status:** DRAFT` and `> **Branch:**` lines, then waves of tasks, each task with `**Files:**` and `**Acceptance:**`. No two tasks in the same wave may touch the same file — waves must be file-disjoint because `/flue-executing` runs a wave's tasks as parallel agents sharing one worktree. Save to `~/c0de/plans/effect-flue/<plan-name>.md`.
   - **Declare expected-red typecheck windows.** Contract-first ordering is normal here: registering an HttpApi group/endpoint (in `packages/shared`) in an early wave makes `HttpApiBuilder.layer(AgentApi)` REQUIRE its handlers, which a later wave wires into `index.ts`. The central per-wave typecheck is then legitimately red between those waves with ONE "group not provided" / unmet-requirement error. When the plan has this shape, add a header line — `> **Expected-red:** central typecheck fails with \`<file:line — exact error signature>\` until Wave N (handler wiring); green by Wave N` — so `/flue-executing` can tell the structural error from a real one.
