...
**A new SIZE-BOUNDED transport (a file upload, a large request/response body, anything with a byte cap) is verified only when a representative IN-BOUNDS LARGE payload — near but under the cap — has actually round-tripped, NOT just a tiny payload plus an over-cap rejection.** The over-cap case is rejected at the size guard (the schema `isMaxLength`, a `Content-Length` check) BEFORE it ever exercises the transport's large-payload path, so `tiny + over-cap-rejected` proves the guard works while proving NOTHING about whether a real big-but-valid payload survives the path it must traverse — a multi-MB body crossing the DO RPC fence, a large tarball through R2, a streamed response. Pick a payload a few × the size of the tiny case and comfortably under the cap (e.g. 2 MB and 9 MB against a 10 MB cap), send it end-to-end, and confirm the bytes actually landed (inspect the destination, not just the `200`). (user-file-upload #146: the verify pass exercised an 18-byte upload and an 11 MB over-cap rejection — but the 11 MB body was rejected at the payload schema BEFORE reaching the Durable Object, so the DO-RPC large-argument path was never tested; the code review flagged a plausible "DO RPC caps args at ~1 MiB" concern that a backfilled 2 MiB / 9 MiB round-trip was needed to actually clear.)
...
Capture the worker URL from the deploy output (currently https://<your-worker>.workers.dev). If it isn't printed, use the `[worker-url]` argument or `BASE_URL` env; if neither exists, ask the user.
...
Then `grep` the dumped DOM for markers unique to your change (panel headings, new component text, expected data values) and `Read` `/tmp/render.png` to eyeball the layout. A near-empty `#root`, a DOM missing your markers, or the old bundle name = the app crashed on mount or a stale build is live — a FAIL even though every endpoint passed. **When the change ADDS a control into an existing row of sibling controls (a button next to an input, an icon in a toolbar, an action in a card header), read the screenshot for ALIGNMENT, not just presence — the new control must match its siblings' height/baseline. "Present + mounts + no errors" passes a control that renders a head taller or shorter than everything beside it, because misalignment is neither a missing marker nor an error string.** (user-file-upload #146: a `size="icon"` paperclip was added beside the composer's pre-existing shadcn `Textarea` — whose base `min-h-16` makes it ~2 rows — so the attach button sat one row tall next to a two-row input; the verification screenshot WAS eyeballed and the button confirmed present, but the height mismatch shipped and the user caught it hands-on after merge, fixed with a `min-h-9` override.)
