...
2. Resolve base:
   ...
   **Unmerged-branch base:** if the plan's `> **Base:**` is an open PR branch (the work amends an unmerged PR), or the target files don't exist on `origin/main` yet, set `BASE_BRANCH` to that branch's ref (`git fetch origin <branch>` → `origin/<branch>`) and create the worktree from it. The work then folds into the open PR. Still a worktree — never a main-copy checkout.
3. **Stale-leftover check first:** if `.claude/worktrees/<plan-name>` already exists on disk, `git worktree add` will refuse — and `git worktree remove` can't clear it (submodule block). Confirm with the user it's a dead leftover, then `rm -rf .claude/worktrees/<plan-name> && git worktree prune` before adding.
   ...
   REPO_URL=$(gh repo view --json url -q .url); [ -n "$REPO_URL" ] && git -C .claude/worktrees/<plan-name> remote set-url origin "$REPO_URL"
   git -C .claude/worktrees/<plan-name> config gc.auto 0
   ```
   If the branch already exists, verify it's based on BASE_BRANCH before reusing.
...
```
Creating worktree: .claude/worktrees/<plan-name> (branch: <branch>, base: origin/main)
...
Do NOT run whole-tree git ops (git reset/checkout ./add -A/stash) — sibling agents share
this worktree. Do NOT run `bun run typecheck` or deploy — `/flue-executing` runs those
centrally after the wave.
```
...
# per changed file, excluding import/export-from lines (import aliases are legit `as`):
grep -vE '^\s*(import|export .* from)' <file> | grep -nE '\bas (const\b|[A-Za-z_])|[^!=]!(\.|\))'
```

The pattern must be `\bas (const\b|[A-Za-z_])`, NOT `as [A-Z]` — an uppercase-only pattern misses `as const` and lowercase casts. Also grep each task's own `**Do not:**` bullets (the backticked code shapes) across the files that task changed. On a hit: fix it, re-run typecheck. If unfixable, `AskUserQuestion`: proceed / abort.
